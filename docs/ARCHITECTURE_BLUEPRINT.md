...
graph BT
  ...
  FSTUDIO[:feature:studio] --> EXPORT & DB & NAV & DS & COMMON
...
classDiagram
    ...
    class RenderClock { media | frameIndex }
...
classDiagram
    ...
    ProjectMStyleBridge ..|> Style : legacy trio REUSE-as-is FBO no fb0-copy
...
classDiagram
    ...
    class PlaybackController { MediaController facade }
...
flowchart LR
  subgraph LIVE
    A[media clock via Media3 sink] -->|tap| R[(SampleRing)] --> RA[ReactiveAnalyzer] --> FR[FeatureRing snapshot] --> MW[ModRouter + SafetyClamp] --> GL[GL frame at surface res drops under load]
  end
  subgraph OFFLINE
    F[file decoder] --> OA[OfflineAnalyzer] --> FT[(FeatureTimeline cache)]
    FT --> FS[FrameStepper idx x 1/fps NO drops] --> EGL[EGL headless FBO] --> PBO[PBO readback] --> SEG[SegmentCache] --> MX[muxer + stitch + LUFS normalize]
  end
```
