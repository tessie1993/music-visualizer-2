...
- 2026-08-26 M4 PORT MERGED (PR #139): 37 legacy DSP files -> :core:audio (dev.synesthesia.core.audio); JTransforms 3.2 pinned; SampleRing gained seqlock-guarded analysis read surface (oldestAvailable/writtenFrames/channelCount/copyInto/planar snapshotLatest) - ONE ring per decision #1, duplicate PcmSink.kt/AnalysisRing.kt deleted; RingReadResult epoch Int->Long. COLLATERAL: foundation modules had NO dependency blocks - repaired billing->coroutines, visualizer->audio, export->billing; blueprint DAG updated (BILLING->EXPORT data-type-only edge). Local assembleDebug+ktlint green; CI green; merged on green. NEXT: M4 remainder = mined characterization tests + ReactiveAnalyzer chain wiring to FeatureRing.
- 2026-08-25 M11 started early (owner request): .github/workflows/synesthesia.yml - PR-gated build+unit-tests+lint+ktlint+debug-APK artifact for synesthesia/** paths; compileSdk resolved from convention-plugin source of truth (literal-free law); artifact upload continue-on-error w/ 7d retention (legacy quota lesson). Self-validates on its own PR.
- 2026-08-25 M7-FOUNDATION code complete: SampleRing hardened (true seqlock: volatile state + post-copy re-check + atomic beginEpoch; absolute positions incl dropped; drop-oldest wrap invariant fixed), TapAudioProcessor owned-wrapper tee (zero-alloc scratch, SR+ch change epoch), PlaybackService wired ExoPlayer-sink-tap-ring, FGS mediaPlayback + POST_NOTIFICATIONS declared. 7/7 unit tests green, assembleDebug green. Gates: architect REVISE->absorbed (2 blocking), data-safety PASS.
- 2026-08-25 FOUNDATION MERGED (PR #136): five seams as code + FOUNDATION_DESIGN.md + L12 cartographer law + blueprint seams ledger. NEXT BLOCK: M7-foundation player (PlaybackService skeleton + library-minimal + PCM tap -> ring), then M4 audio engine port. All loop state lives in this file - nothing else is needed to resume.
