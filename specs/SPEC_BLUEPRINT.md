...
- Safety architecture (D-SAFE-1, AAA-review hardened): **TWO-STAGE** flash control, and Stage 2 is ORDERED AFTER EVERY PIXEL-MODIFYING STAGE. Stage 1 — param-space WCAG ceiling applied LAST inside ModRouter (≤3 flashes/s equivalent, non-defeatable, not a setting). Stage 2 — `FlashBudget` output-space pass measuring FINAL composited luminance, executing after EVERY pixel-modifying stage in BOTH chains: live it runs AFTER FSR1/RCAS upscaling (sharpening overshoot is luminance); offline it runs AFTER in-engine captions/overlays. Captions are rendered IN-ENGINE (GL text pass inside the clamp) — Media3 `OverlayEffect` is reserved exclusively for the static free-tier watermark (non-flash by nature). Post-clamp passes restricted to non-additive or budget-aware effects. Clamp is cadence-aware: transitions→Hz computed from ACTUAL emitted fps. An arch-test asserts the pass order in both pipelines (fails build if any pixel-modifying pass lands after FlashBudget). Optional comfort controls operate BELOW stage 1, default OFF, off = exact no-op.
