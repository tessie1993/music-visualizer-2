...
Goal: the timbre half of V2-3-05 — MFCC and delta MFCC, six-band spectral contrast, and
timbre flux — as oracle-validated `audio-core` nodes, the way V2-3-03a landed the spectral
descriptors. Chroma, pitch and key stay on the app-side `Chromagram`/`KeyDetector` incumbents;
their migration with corpus proof is V2-3-05b.

User-visible effect: none. New nodes with no production consumer; wiring them into the
analyzer and the ABI belongs to the slice that also sizes the pitch/timbre branch (§5.3's
4096 window), not this one.

In scope: `MelBank` (HTK-formula triangular filters over the power spectrum, unit-peak,
matching `librosa.filters.mel(htk=True, norm=None)` exactly), `Mfcc` (10·log10 with the
oracle's 1e-10 floor, orthonormal DCT-II, first 13 coefficients, plus frame-difference delta
and the L2 timbre flux over c1..c12), `SpectralContrast` (six octave bands from 200 Hz,
peak-minus-valley in dB over the top/bottom 2% of bins); corpus generator version 3 with
per-frame `mfcc`, `spectralContrast` and `timbreFlux` expectations recomputed explicitly in
numpy; `TimbreOracleTest` plus degenerate-input unit tests.

Out of scope: chroma/pitch/key migration (V2-3-05b); production wiring and the ABI;
Savitzky-Golay delta (the delta here is the plain causal frame difference, documented as
such); the device benchmark the phase owes — **still carried, no device here.**

Files expected to change: `tools/oracle/generate_corpus.py`,
`app/src/test/resources/corpus/*`,
`engine/audio-core/src/main/kotlin/dev/geode/engine/audio/{MelBank,Mfcc,SpectralContrast}.kt`
and their tests, `app/src/test/java/dev/geode/analysis/TimbreOracleTest.kt`.

Compatibility contract: nothing existing changes. The corpus regenerates byte-identically
(same seeded fixtures, same library versions — verified before regenerating); only the
manifest grows new expectation blocks. The raw-PCM `PcmSink` fan-out is untouched.

External source/provenance entries: none new. **librosa** (ORACLE) supplies the STFT and the
mel filterbank for the expectations; the manifest states each formula once, recomputed in
numpy. Formulas are the published ones: HTK mel scale, DCT-II MFCC (Davis & Mermelstein),
octave-band spectral contrast (Jiang et al., *Music type classification by spectral
contrast feature*, ICME 2002). No code, naming, layout or constant table from any
implementation.

Tests written first: yes — the oracle sweep and the degenerate-input tests exist before the
nodes compile.

Benchmark or visual evidence: not applicable; nodes have no production consumer yet. The
comparison tolerance was MEASURED, per the V2-3-03a bandwidth precedent: probing at 0.02
found the corpus worst case at 2.2% relative on ordinary material and 5.3% on the splice
fixture, whose contrast band 3 holds only crossover leakage — float32-vs-float64 exactly
where the log amplifies it. Tolerance 0.08 clears that, and three fault injections prove it
still catches everything it must.

Rollback: revert the one commit.

Risks: 0.08 relative is looser than the other descriptors' tolerances; the injections bound
what can hide under it (nothing structural — a single alpha off-by-one already fails).
Cost is unmeasured in situ, as with every node in this phase before wiring.

Commands and results: `:engine:audio-core:test` 149 tests, `:app:testDebugUnitTest`
(including the three-sweep `TimbreOracleTest`, ~7,800 mfcc comparisons), ktlint and
`:app:lintDebug` all green. Corpus regenerated byte-identically (.pcm untouched, manifest
grew the timbre block; library versions matched the recorded ones before regenerating).

Review findings: two.

**The first mel-scale fault injection was vacuous, and looked like evidence.** Replacing
`2595 log10(1 + f/700)` with `1127 ln(1 + f/700)` is the SAME scale in different units — and
since the filter edges are ratios of the top mel, any constant factor cancels entirely. The
sweep passing under it said nothing. The real injection (break frequency 700 -> 1000 Hz)
fails the sweep at once, as do the DCT-norm and alpha-count injections. A fault injection
has to be checked for actually being a fault.

**Tolerance was set by measurement, twice.** 0.05 — twice the worst case seen at the first
probe — still failed on one frame of the splice fixture, because the first probe's two
failures were not the distribution's tail. The recorded number comes from the full sweep.

Commit: `feat(audio-core): MFCC, spectral contrast and timbre flux, oracle-validated`.

Next slice: V2-3-05b — chroma, pitch and key against the corpus — or the owed device
benchmark, whichever the environment allows.
