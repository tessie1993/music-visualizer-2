 ...
 * Mounted by SettingsScreen inside the collapsible "Playback" section, so it
 * adds no header of its own — just crystal-styled rows.
 */
@Composable
fun PlaybackSettingsSection(viewModel: PlayerViewModel) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
        CrystalSliderRow(
            ...
            prefs.speed,
            0.5f..2f,
            onChange = { viewModel.setPlayerPrefs(prefs.copy(speed = PlaybackMath.snap(it, 0.05f))) },
            valueText = "%.2fx".format(prefs.speed),
        )
        CrystalSliderRow(
            "Pitch (0 = normal)",
            prefs.pitchSemitones,
            -6f..6f,
            onChange = { viewModel.setPlayerPrefs(prefs.copy(pitchSemitones = PlaybackMath.snap(it, 0.5f))) },
            valueText = "%.1f st".format(prefs.pitchSemitones),
        )
        ...
        Column {
            CrystalOverline("Sleep timer", color = MaterialTheme.colorScheme.onSurfaceVariant)
            Row(
...
private fun PlaybackSwitchRow(
    ...
    Row(
        ...
        Switch(checked = checked, onCheckedChange = onChange, colors = crystalSwitchColors())
    }
}
