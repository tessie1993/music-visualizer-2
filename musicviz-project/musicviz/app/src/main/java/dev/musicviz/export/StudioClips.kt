 ...
 *
 * Scoped to `Movies/MusicViz`, the shelf both exporters write to. Two reasons
...
object StudioClips {
    ...
    fun list(context: Context): List<StudioClip> {
        ...
        val args =
            arrayOf(
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) "Movies/MusicViz%" else "%/Movies/MusicViz/%",
            )
        runCatching {
