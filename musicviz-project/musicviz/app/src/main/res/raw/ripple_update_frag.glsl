...
//   v += c^2 * dt * laplacian(h) / dx^2;  v *= damping;  h += v * dt
...
uniform float uDt;
uniform float uDamping;      // per-substep decay factor
...
void main() {
    ...
    // MAX_HEIGHT rail (mirrored in RippleMath.waveStep): drop spam can pile
    // amplitude faster than damping drains it; never let half-floats blow up.
    float h = clamp(hv.x + v * uDt, -8.0, 8.0);
