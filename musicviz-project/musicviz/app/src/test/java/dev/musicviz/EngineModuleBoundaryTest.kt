    ...
    fun `the pure modules are pure by construction, not by convention`() {
        ...
        listOf("audio-core", "visual-core").forEach { module ->
            assertTrue(
                ...
                buildFileOf(module).readText().contains("""id("musicviz.jvm-library")"""),
            )
        }
    }
        ...
        val offenders =
            modules.flatMap { module ->
                sourcesOf(module)
                    .filter {
                            file ->
                        Regex("""^import dev\.musicviz\.(ui|data|playback)\.""", RegexOption.MULTILINE).containsMatchIn(file.readText())
                    }
                    .map { "$module/${it.name}" }
            }
        assertEquals(
