...
import dev.geode.engine.audio.RingReadResult
import dev.geode.engine.audio.RingReader
...
class PlaybackEngineTest {
    ...
    fun `the capture controller is handed the session's own sink`() {
        ...
        val source =
            java.io.File(dev.geode.ParamSurface.moduleRoot, "app/src/main/java/dev/geode/ui/PlayerViewModel.kt")
                .readText()
        assertTrue(
    ...
    fun `a player with nothing loaded is not worth keeping a service alive for`() {
        ...
        // service stays up. Idle has to read as "no", or Geode would leave a
    ...
    fun `a media-button receiver exists, or playback resumption never happens`() {
        ...
        // Geode never appeared in the System UI media carousel after its
    ...
    fun `the capture service keeps its own foreground type, so the two can run at once`() {
        ...
        val capture =
            ctx.packageManager.getServiceInfo(
                ComponentName(ctx, dev.geode.audio.PlaybackCaptureService::class.java),
                0,
            )
        assertEquals(
