...
class VmBehaviorTest {
     ...
     */
    private fun awaitPersistedLiveState(matches: (dev.geode.data.Preset) -> Boolean) {
        val prefs =
            ApplicationProvider
                .getApplicationContext<Application>()
                .getSharedPreferences("geode-viz", android.content.Context.MODE_PRIVATE)
    ...
    fun live_customization_survives_restart() {
        val first = vm()
        first.selectScene(dev.geode.render.scene.SceneIds.FLUID)
        ...
        assertEquals(dev.geode.render.scene.SceneIds.FLUID, s.sceneId)
