...
class LiveStateWriteTest {
    ...
    fun `a single change still lands without another one to push it`() {
        ...
        val v = PlayerViewModel(context)
        v.selectScene(dev.geode.render.scene.SceneIds.WATER)
        val persisted = awaitPersisted { it.sceneId == dev.geode.render.scene.SceneIds.WATER }
        assertEquals(dev.geode.render.scene.SceneIds.WATER, persisted?.sceneId)
    }
    ...
    fun `intelligence and analysis fields are not clobbered by a colour roll`() {
        val v = PlayerViewModel(context)
        v.setRandomizeColors(true)
        v.setIntelligenceMode(dev.geode.analysis.IntelligenceMode.AUTO)
        v.randomStepNow()
        assertEquals(dev.geode.analysis.IntelligenceMode.AUTO, v.vizState.value.intelligenceMode)
