...
android {
    ...
    defaultConfig {
        ...
        minSdk = 26
        targetSdk = 36
        // 25 is already published from main; this merge adds background
        // playback and the 16 KB libraries on top of 1.1.0.
        versionCode = 26
        versionName = "1.1.0"
        ndk {
...
tasks.withType<Test>().configureEach {
    ...
    // A deadlocked Robolectric test produces no output at all, so without a
    // bound it silently burns the whole CI job timeout and reports nothing
    // useful — that is how the ExoPlayer.release() deadlock presented. Fail
    // fast instead: the suite normally finishes in about two minutes.
    // Duration is imported at the top because inside a build script `java`
    // resolves to the Java plugin extension, shadowing the package root.
